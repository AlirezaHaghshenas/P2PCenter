...
void CClientMgr::OnUdpShootHole( P2S_COMMAND::CCmdShootHole* pCmd ,unsigned long ulIP, unsigned short usPort)
{
	...
	if (it != m_map.end())
	{
		...
		{
			// Do a simple comparison check SESSION
...
void CClientMgr::OnConnect( P2S_COMMAND::CCmdConnect* pCmd )
{
...

	{
		...
		if (it != m_map.end())
		{
			...
			// This UserID will be automatically generated by the database ID, temporarily borrow SESSION
...
void CClientMgr::OnLoginChannel( P2S_COMMAND::CCmdLoginChannel* pCmd )
{
	...
	// Notify other users this user enters the room Room
...
void CClientMgr::OnLogoutChannel( P2S_COMMAND::CCmdLogoutChannel* pCmd )
{
	...
	// Notify other users of this user leaves the room Room
	{
		CKAutoLock ll( &m_KCritSec );
	...
	{
		// Such as passive, closed channels and notify other users to be written
...
void CClientMgr::OnGetPeers( P2S_COMMAND::CCmdGetPeers* pCmd )
{
	...
	if ( pCmdGetPeersRet )
	{
		...
		{
			pClient = it->second;
			if ( pClient->m_peerInfo.ulID != pCmdGetPeersRet->m_nUserID && pClient->IsCanWork() )	//Exclude yourself
			{
				...
				if ( itpeer != mapPeers.end() )
				{
					...
					{
						//Achieved only source of non-source client
...
{
	// Database updated every ten minutes Tick
