...
int CClientApp::OnConnect()
{
	return 0;
}

int CClientApp::OnDisConnect()
{
	return 0;
}
		...
		else
		{
			...
			if ( pMediaFile )
			{
				if ( pMediaFile->InitFile( fFind.GetFilePath().GetBuffer(0), fFind.GetFileName().GetBuffer(0) ) )
				{
					...
					CKLog::WriteLog( LOG_TYPE_DEBUG, "%s, Count: %u\n\t%s?filehash=%s&filesize=%I64u", fFind.GetFilePath(), m_MediaFileMgr.GetCount(), 
...
DWORD CClientApp::PublishFiles()
{
	...
	char szUrl[1024];
	sprintf( szUrl, "p2p://www.videobbs.com:8901/%s?agnet=agentname&upuserid=0001&userid=10002&nodename=%s&filehash=%s&filesize=%I64u&filepath=%s&filename=%s&modifydate=%u", 
		pMediaFile->m_sFileName.c_str(), pMediaFile->m_sNodeName.c_str(), pMediaFile->m_sFileHash.c_str(), 
