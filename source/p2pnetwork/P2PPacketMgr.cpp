...
bool CP2PPacketMgr::PacketArrived( DWORD dwBlockIndex, char* pBuffer )
{
	...
	if ( it != m_map.end() )
	{
		...
		{
			CKLog::WriteLog( LOG_TYPE_WARNING, "New block to, but non-state request, the state is£º %d, Block No.£º%d", 
...
void CP2PPacketMgr::UpdatePacketStatus( const map<DWORD, DWORD>& mapRequests, PACKET_STATUS status)
{
	...
	//CKLog::WriteLog( LOG_TYPE_DEBUG, "Number of states have been updated£º %d", dwUpdates);
}
